def getAno (string)
  return string.split[3].to_i
end

def calificar (movies,name)

  if(!movies.keys.include?name)
    puts "La pelicula #{name} no se encuentra, verifique mayúsculas y minúsculas"
    return
  end
  str = movies[name][:votes]
  calificacion=0
  str.each do |x|
    if(x.eql?:good)
      calificacion+=1
...
puts "Los generos de peliculas son:"
generos ={}
movies.each do |x|
  #puts x[1][:realease_date]
  if !generos.include?x[1][:genre]
...
nombreGeneros=generos.keys
puts nombreGeneros


generos.each do |gen|
  puts "el genero #{gen[0]} posee #{gen[1]} peliculas"
end




puts "ingrese un año minimo"
anoMin=gets.chomp.to_i
puts "ingrese un año máximo"
anoMax=gets.chomp.to_i

peliculasPorAno=[]

movies.each do |x|
  if getAno( x[1][:realease_date]) >= anoMin and getAno( x[1][:realease_date])<=anoMax
    peliculasPorAno.push(x[0])
  end
end

if peliculasPorAno.nil?
  puts "No se encuentran peliculas entre el año #{anoMin} y el #{anoMax} son:"
else
  puts "las peliculas que se encuentran entre el año #{anoMin} y el #{anoMax} son:"
  puts peliculasPorAno
end




#puts movies["Johnson Trail"][:votes]

puts "Ingrese el nombre de una pelicula a calificar"
nombrePelicula = gets.chomp
calificar(movies,nombrePelicula)